...
public class CongestionService {
    ...
    public List<CongestionDto> getCongestionByRoute(String routeName, String date) {
        ...
        String sql =
            ...
            "    CASE WHEN r.재차량 < 0 THEN 0 ELSE ROUND(r.재차량 / 19.0) END AS 재차량, " +
            "    MAX(CASE WHEN r.재차량 < 0 THEN 0 ELSE ROUND(r.재차량 / 19.0) END) OVER (PARTITION BY r.노선명) AS 최대재차량, " +
            "    ROUND(" +
            "        CASE WHEN r.재차량 < 0 THEN 0 ELSE ROUND(r.재차량 / 19.0) END * 100.0 " +
            "        / NULLIF(MAX(CASE WHEN r.재차량 < 0 THEN 0 ELSE ROUND(r.재차량 / 19.0) END) OVER (PARTITION BY r.노선명), 0)" +
            "    , 1) AS 상대혼잡도, " +
            "    b.위도, " +
            "    b.경도, " +
            "    COALESCE((" +
